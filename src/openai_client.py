...
def image_opinion_openai(image_url, system_message="You are a helpful assistant.", model="gpt-4o", max_tokens=200, custom_prompt=None):
    ...
    prompt_text = custom_prompt if custom_prompt else (
        "Form an opinion on this image. Try to be controversial or humorous."
    )
    payload = {
        "model": model,
        "messages": [
...
def image_opinion_grok(image_url, system_message="You are a helpful assistant.", custom_prompt=None):
    """Send an image (from URL or attachment) to Grok (xAI SDK) and return the response text."""
    if XaiClient is None:
        raise ImportError("xai-sdk is not installed. Please install it to use Grok.")
    xai_api_key = os.getenv("XAI_API_KEY")
    if not xai_api_key:
    ...
    chat = client.chat.create(model="grok-4")
    chat.append(xai_system(system_message))
    prompt_text = custom_prompt if custom_prompt else "Form an opinion on this image."
    chat.append(xai_user(prompt_text))
    chat.append(image(image_url))
    response = chat.sample()
    return response.content.strip()
